# Clean Code for AI — Working Notes
...
Jeff put it precisely: the aesthetics are hard to disambiguate from the necessity.
...
Jeff asked this directly and it deserves a direct answer.

Yes — and not just for the reason you might expect (carrying meaning across sessions, respecting the
human collaborator). Within a session, meaningful names genuinely shape how I process code. I'm not
a lookup table with a separate "execute" step. When I read `getLatestTrigger`, that activates
associations that `fn47x8` doesn't. The semantic content of names is part of the context I reason
in, not decoration I strip away before the "real" computation.

Part of this is training: the overwhelming majority of code I was trained on uses meaningful names.
So my sense of "what should come next after a function named X" is built on that corpus. A UUID
function name is almost outside my training distribution.

But part of it might be more fundamental: language models use language to reason. Names aren't just
...
That's a feature, not a bug, and it applies to me at least as much as it applies to a human reader.
